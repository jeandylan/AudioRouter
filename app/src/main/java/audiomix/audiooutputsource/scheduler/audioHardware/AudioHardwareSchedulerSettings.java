...
public enum AudioHardwareSchedulerSettings {
    ...
    final static String TAGAu="CON_AUDIOScheSet";
    final static String PREF_NAME="AudioHardwareSchedulerSettings";
    ...
    AudioHardwareSchedulerSettings(boolean on, int startTime, int endTime) {
        this.on = on;
    ...
    public static boolean  canApply(){ //cannot return nor
       checkIsSettingLoaded();
        AudioHardwareSchedulerSettings todaySetting=getTodayScheduler();
        ...
        Log.d(TAGAu,"today scheduler: "+calendar.get(Calendar.DAY_OF_WEEK)+" is on:"+todaySetting.isOn()+" start: "+ todaySetting.getStartTime()+ "end: "+todaySetting.getEndTime());
    ...
    public static  AudioHardwareSchedulerSettings getTodayScheduler(){
        checkIsSettingLoaded();
    ...
    public static Long  getNextUpdateTimeInMillis(){ //return null if no scheduler, -1 if always on
      checkIsSettingLoaded();
        AudioHardwareSchedulerSettings todayScheduler=getTodayScheduler();
        if(todayScheduler.getStartTime()==-1) return new Long(-1);
    ...
    public static AudioHardwareSchedulerSettings getScheduler(int dayId){
        checkIsSettingLoaded();
        switch (dayId) {
            case Calendar.SUNDAY:
                return sunday;
    ...
    public static  void saveSetting(Context ctx){
        ...
        AudioHardwareSchedulerSettings daySetting;//current audio hard. setting i.e the unsaved instance
    ...
    public static void loadSetting(Context ctx){
        ...
        int[] days ={Calendar.SUNDAY,Calendar.MONDAY,Calendar.TUESDAY,Calendar.WEDNESDAY,Calendar.THURSDAY,Calendar.FRIDAY,Calendar.SATURDAY};
        AudioHardwareSchedulerSettings daySetting; //default instance as is in memory
        for(int i=0;i<days.length;i++){
            daySetting= getScheduler(days[i]);//get current unsaved enum based on id,iterate trow all day of week
    ...
    public static void checkIsSettingLoaded(){
        if(!isSettingLoaded()) try {
            Log.d(TAGAu,"hardware setting not loaded");
